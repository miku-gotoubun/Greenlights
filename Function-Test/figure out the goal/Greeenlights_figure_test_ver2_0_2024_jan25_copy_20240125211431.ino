...
//ゴールの設定
double golelat = 33.622484;
double golelot = 133.719479;  //Latitude: 33.6204767, Longitude: 133.7194283
...
void figure_measurement_control() {
  if((Mag_Degree>=0 && gps_ang>=0) || (Mag_Degree<0 && gps_ang<0)) {
        control_degree=Mag_Degree-gps_ang;
        if(control_degree>=0) {
        //左回転
      ...
      ledcWrite(1,160);
      ledcWrite(0,0);
      delay(abs(control_degree)*10+100);
      ...
      else if(Mag_Degree>0 && gps_ang<0) {
        control_degree=Mag_Degree-gps_ang;
        if(control_degree<=180) {
          //左回転
      digitalWrite(INB1,HIGH);
      digitalWrite(INB2,LOW);
      ledcWrite(1,160);
      ...
      delay(1000);
        ...
        else if(control_degree>180) {
          control_degree=360-(Mag_Degree-gps_ang);
          //右回転
      digitalWrite(INA1, LOW);
      digitalWrite(INA2, HIGH);
      ledcWrite(0, 160);
...
void loop() {
  while (gpsSerial.available() > 0) {
    if (gps.encode(gpsSerial.read())) {
      ...
      */
      ...
      while(abs(Mag_Degree-gps_ang)>30 && (abs(Mag_Degree-gps_ang)<330)) {
        figure_measurement_control();
        //BMX055 磁気の読み取り
      BMX055_Mag();
      Serial.print("Mag= ");
      Serial.print(xMag);
      ...
      if(abs(Mag_Degree-gps_ang)<=30 || (Mag_Degree-gps_ang>=330 && Mag_Degree-gps_ang<=360)){
        break;
      }
  ...
  /*else if(Mag_Degree>=180 && Mag_Degree<=-180) {
    //停止
      digitalWrite(INA1, LOW);
      digitalWrite(INA2, LOW);
      ledcWrite(0, 0);
  ...
  */
  ...
  //delay(100);
/*
  while(gps_ang<=30 && gps_ang>=-30) {

  if(gps_ang>30 && gps_ang<=180) {
      //左回転
      digitalWrite(INB1, HIGH);
      digitalWrite(INB2, LOW);
      ledcWrite(1, 160);
      ...
      ledcWrite(0, 0);
      delay(abs(gps_ang)*10);
      Serial.println("GPS左回転");
      //停止
      digitalWrite(INA1, LOW);
      digitalWrite(INA2, LOW);
      ledcWrite(0, 0);
  ...
  else if(gps_ang<-30 && gps_ang>=-180) {
      //右回転
      digitalWrite(INA1, LOW);
      digitalWrite(INA2, HIGH);
      ledcWrite(0, 160);
...
/*
  if(Distance<=2) {
    //停止
      ...
      ledcWrite(1, 0);
      delay(1000);
    Serial.println("STOP near the goal");
  delay(100);
  }
