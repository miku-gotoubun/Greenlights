...
//ゴールの設定
double golelat = 33.6203177;
double golelot = 133.7193042;  //33.62031779578117, 133.71930427584002
...
void figure_measurement_control() {//回転量を計算する
   BMX055_Mag();
   X = xMag - 72.5;
   Y = yMag - 168;
  ...
  if((gps_ang>=0) && (Mag_Degree<0)) {
    ...
    if((abs(gps_ang) > abs(Mag_Degree)) && (control_degree>30)) {
      ...
      if((control_degree<=180) && (control_degree>30)) {
        Greenlights_go=2;//左回転
      }
      else if((control_degree>180) && (control_degree<330)) {
        control_degree=360-control_degree;
        Greenlights_go=1;//右回転
      }
...
//void GPS_and_BMX_measure() {}

void Greenlights_moving() {
  //右回転する条件
  if (Greenlights_go == 1) {
    //右回転
    ...
    ledcWrite(1, 0);
    delay(abs(control_degree)*10+200);
  ...
  //直進する条件
  else if ((Greenlights_go == 1) && (Distance>5)) {
    //直進
    digitalWrite(INA1, LOW);
      digitalWrite(INA2, HIGH);
      ledcWrite(0, 200);
