...
Serial.println("秒");
      ...
      //北を0度としたときの目的地の角度 float a_ang=atan2((gps_lng-def_lng),(gps_lat-def_lat))*180/PI;
       float a_ang=atan2(golelat-(gps.location.lat()),(golelot-gps.location.lng()))*180/PI;

       
      Serial.print("Direction = ");    //目的地Aの方角(°）
      Serial.println(a_ang,4);
...
//機体の向き検知
  //オフセット補正　float Mag_offsetX = 0.0, Mag_offsetY = 0.0;
  X=xMag-75;
  Y=yMag-65;
  if(X>=0 && Y>=0){
  ...
  //機体から見てゴールの方角はどこにあるのか
  float goal_degree=a_ang+90-D;
